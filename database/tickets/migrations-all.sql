...
DO $EF$
BEGIN
    IF NOT EXISTS(SELECT 1 FROM tickets.__ef_migrations_history WHERE "migration_id" = '20260925185723_AddAuditLog') THEN
    CREATE TABLE tickets.audit_log (
...
BEGIN
    IF NOT EXISTS(SELECT 1 FROM tickets.__ef_migrations_history WHERE "migration_id" = '20260925185723_AddAuditLog') THEN
    CREATE INDEX ix_audit_log_tenant_id_entity_type_entity_id ON tickets.audit_log (tenant_id, entity_type, entity_id);
    END IF;
END $EF$;
...
BEGIN
    IF NOT EXISTS(SELECT 1 FROM tickets.__ef_migrations_history WHERE "migration_id" = '20260925185723_AddAuditLog') THEN
    CREATE INDEX ix_audit_log_tenant_id_occurred_at ON tickets.audit_log (tenant_id, occurred_at);
    END IF;
END $EF$;
...
BEGIN
    IF NOT EXISTS(SELECT 1 FROM tickets.__ef_migrations_history WHERE "migration_id" = '20260925185723_AddAuditLog') THEN
    INSERT INTO tickets.__ef_migrations_history (migration_id, product_version)
    VALUES ('20260925185723_AddAuditLog', '10.0.10');
    END IF;
END $EF$;
COMMIT;
